...
func TestWeaponPAtk_Combat(t *testing.T) {
	...
	target, err := model.NewPlayer(2, 101, 201, "Target", 10, 0, 0)
	if err != nil {
		t.Fatalf("NewPlayer failed: %v", err)
	}
	...
	damageNoWeapon := targetHPBefore - targetHPAfter

	t.Logf("Attack without weapon: damage=%d (HP: %d → %d)", damageNoWeapon, targetHPBefore, targetHPAfter)
	...
	targetHPAfter = target.CurrentHP()
	damageWithWeapon := targetHPBefore - targetHPAfter

	t.Logf("Attack with sword (pAtk=8): damage=%d (HP: %d → %d)", damageWithWeapon, targetHPBefore, targetHPAfter)
...
func TestArmorPDef_Combat(t *testing.T) {
	...
	attacker, _ := model.NewPlayer(1, 100, 200, "Attacker", 10, 0, 0)
	attacker.SetLocation(model.NewLocation(0, 0, 0, 0))
	...
	// Create defender
	defender, _ := model.NewPlayer(2, 101, 201, "Defender", 10, 0, 0)
	...
	damageNude := defenderHPBefore - defenderHPAfter

	t.Logf("Defender nude: damage=%d (HP: %d → %d)", damageNude, defenderHPBefore, defenderHPAfter)
	...
	defenderHPAfter = defender.CurrentHP()
	damageWithArmor := defenderHPBefore - defenderHPAfter

	t.Logf("Defender with armor (pDef=100): damage=%d (HP: %d → %d)", damageWithArmor, defenderHPBefore, defenderHPAfter)
