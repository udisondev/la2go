...
func TestVisibilityCache_EndToEnd(t *testing.T) {
	...
	region1 := worldInstance.GetRegion(regionX1, regionY1)
	if region1 == nil {
	...
	for i := range objectCount {
		obj := model.NewWorldObject(uint32(i+1), "TestNPC", loc1)
	...
	// Wait for first batch update (50ms interval + buffer)
	...
	// Note: May contain more objects from surrounding regions or previous tests
	...
	region2 := worldInstance.GetRegion(regionX2, regionY2)
	if region2 != nil {
		for i := range 5 {
			obj := model.NewWorldObject(uint32(i+100), "NPC2", loc2)
			region2.AddVisibleObject(obj)
		}
	}

	// Wait for cache update (should detect region change)
...
func TestVisibilityCache_MultiplePlayersStressTest(t *testing.T) {
	...
	for i := range 500 {
		...
		region := worldInstance.GetRegion(regionX, regionY)
		if region != nil {
	...
	// Wait for several batch updates
	...
	// Wait for cache updates after movement
